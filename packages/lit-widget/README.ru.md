...
Привязка целей в классе виджета осуществляется либо с помощью дектораторов [`@target`](#target) и [`@targets`](#targets), либо через статические свойства [`targets`](#static-targets) и [`targetsAll`](#static-targetsall).
...
**LitWidget** автоматически добавляет указанные как обработчики событий *публичные* методы класса в качестве обработчиков событий при подключении к DOM-элементу (в `connectedCallback`) и отключает их при отключении от DOM-элемента (в `disconnectedCallback`).
...
Указать метод класса как обработчик какого-то DOM-события можно двумя способами, с помощью декторатора [`@onEvent()`](#синтаксис-onevent) или свойства класса [`events`](#синтаксис-свойства-events).
...
Вы так же можете передать существующий HTML-элемент или объект `window` или `document`, чтобы добавить обработчик события, например, к `document.body` или `window`:
...
```
В приведенном выше примере обработчик `itemClick(...)` сработает при нажатии только на пункты списка `<div class="list-item">...</div>`, но не на `<div class="list-separator"></div>`.
...
с суффиксом `'<delay>ms'`, поддерживаемые суффиксы: ms - миллисекунды, s - секунды, m - минуты.
...
Функция-обертка для применения дополнительных декораторов к обработчику событий;
может быть полезно, например, для применения декоратора `debounce` с задержкой, установленной во время выполнения:
```js
@onEvent(..., wrapper: (fn, self) => debounce(fn, self.delay * 1000) )
```

Первый параметр, передаваемый в функцию — это метод обработчика события,
второй является ссылкой на экземпляр класса.
...
`handler` - должен указывать на метод класса или функцию - обработчик события.
...
**LitWidget** в определении события позволяет указать вместо имени события объект состоящий из двух полей, `eventName` и `isMatch`:
```js
{
  eventName: 'keydown',
...
* **eventName** - это название события;
* **isMatch** - это функция, которая принимает в качестве параметра событие и возвращает булевое значение, если функция вернула `true`, то обработчик события сработает.

В приведенном ниже примере обработчик события `keydown` сработает только при нажатии на кнопку `Esc`:
```js
class SampleWidget extends LitWidget {
