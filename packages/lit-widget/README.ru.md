...
Вы так же можете передать существующий HTML-элемент или объект `window` или `document`, чтобы прикрепить обработчик события, например, к `document.body` или `window`:
...
class HelloWidget extends LitWidget {
  ...
  events = [
    ...
    // Обработчик события с ограниченной часототой срабатывания
    {target: 'search-field', event: 'input', handler: this.typing, debounce: '500ms'},

    // Обработчик события с preset
