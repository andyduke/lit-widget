...
Веб-компоненты (пользовательские элементы) определены в JavaScript, включая их HTML и CSS. Они программно добавляются в DOM через API. К тому времени, когда браузер интерпретирует и выполнит этот код, есть большая вероятность, что конвейер рендеринга уже поместил пользовательский элемент на экран. Поскольку он не знает об элементе в первый раз, он отобразит его без предполагаемого стиля. После того, как JavaScript определения пользовательского элемента выполнен и браузер, следовательно, знает о правилах CSS, которые применяются к этому элементу, он может обновить представление.
