...
Для этого необходимо установить свойство `sharedStyles` в `true`, **LitWidget**  будет отслеживать изменения в стилях страницы и при появлении нового тэга со стилями автоматически сделает его доступным и во всех виджетах со включенным `sharedStyles`, так что если Вы подключаете какие-то API, например встраиваете карты, то их стили будут доступны в Shadow DOM виджетов. Если же такое поведение необходимо отключить, то надо установить свойство `sharedStyles` в `false`.
